...
-- Policy 1: Users can view their own basic profile data (excluding email for now)
...
-- Add additional security logging for email access attempts
...
AS $$
BEGIN
  -- Log all SELECT operations that access email column
...
-- Create trigger to log email access (for monitoring)
DROP TRIGGER IF EXISTS log_profile_email_access ON public.profiles;
CREATE TRIGGER log_profile_email_access
  AFTER SELECT ON public.profiles
  FOR EACH ROW
  WHEN (pg_trigger_depth() = 0) -- Prevent infinite recursion
  EXECUTE FUNCTION public.log_email_access();
