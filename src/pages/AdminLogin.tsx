  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user) {
        ...
        if (ensureError) {
          console.error('Profile verification error:', ensureError);
          await supabase.auth.signOut();
          toast({
            title: "Profile Error",
            description: "Unable to create profile. Please contact your administrator.",
            variant: "destructive",
          });
          setIsLoading(false);
          return;
        ...
        if (!ensured || ensured.length === 0) {
          ...
          toast({
            title: "Access Denied",
            description: "No profile data returned. Please contact your administrator.",
            variant: "destructive",
          });
          setIsLoading(false);
          return;
        ...
        const role = ensured[0]?.role as string;

        if (role !== 'admin') {
          await supabase.auth.signOut();
          toast({
            title: "Access Denied",
            description: `Your role is "${role}". Only admin accounts can access this portal.`,
            variant: "destructive",
          });
          setIsLoading(false);
          return;
        ...
        // Admin verified - show success and navigate
        toast({
