...
const AdminBirthdayManager = () => {
  ...
  const handleSave = async () => {
    try {
      ...
      if (editingBirthday) {
        ...
        if (error) throw error;
        await logAdminAction(`Updated Birthday: ${formData.name}`);
        ...
        if (error) throw error;
        await logAdminAction(`Added Birthday: ${formData.name}`);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      if (error) throw error;
      await logAdminAction(`Deleted Birthday: ${birthdayToDelete?.name || 'Unknown'}`);
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (error) throw error;
      await logAdminAction(`Imported ${data.length} Birthdays from CSV`);
      toast({ 
        title: "Success", 
